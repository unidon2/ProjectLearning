...
public class Client extends JFrame implements MouseListener,ActionListener {
	...
	public void setvalue() {
		input_restTimenum.setText("1000");
		access_restTimenum.setText("1000");
	}
	...
	public void changeTurn() {
		player.setColor(player.getColor()*(-1));
	}
	public void setInputTime(int n) {
		input_time = n;
	}
	public void prepare() {
			...
			try{
				while(true) {//データを受信し続ける
					...
					if (inputLine != null){//データを受信したら
						//System.out.println(inputLine+"を受信しました");
	...
	public void receiveMessage(String msg){	// メッセージの受信
		//System.out.println(msg+"を受信しました.");
		if (msg.equals("b")) { //先手後手情報の受信
			player.setColor(1);
			color.setText("あなたの色 : 黒");
			System.out.println("Color decided");
			match = true;
			return;
		}
...
public void setLimitTime(/*Player player*/) {
 ...
 public void accept(/*Player player*/) {
	...
	public static void main(String args[]){
		//Player player = null;Othello game = null;
		Client oclient = new Client(); //引数としてオセロオブジェクトを渡す
		oclient.prepare();
		oclient.waitMatching(); //本プログラムの開始
	}
}
