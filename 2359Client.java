...
public class Client extends JFrame implements MouseListener,ActionListener {
	...
	private Container c,cont=new Container(),conts=new Container(),contsm =new Container(),conres; // コンテナ
	...
	public void detect(String str) {
		...
		jfd.setSize(370,600);//ウィンドウのサイズを設定
	...
	public void matching() {
		...
		jfmat = new JFrame();
		jfmat.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//ウィンドウを閉じる場合の処理
		jfmat.setTitle("マッチング中");
		jfmat.setSize(400, 300);//ウィンドウのサイズを設定
		contsm = jfmat.getContentPane();
		contsm.setLayout(null);
		jfmat.setLocationRelativeTo(null);
		
		jlmat1 = new JLabel("マッチング中");
		jlmat1.setBounds(0,90,400,30);
		jlmat1.setFont(new Font("Century",Font.PLAIN,20));
		jlmat1.setHorizontalAlignment(JLabel.CENTER);
		
		jlmat2 = new JLabel("対局相手が見つかるまでお待ちしてください");
		jlmat2.setBounds(0,130,400,30);
		jlmat2.setFont(new Font("Century",Font.PLAIN,15));
		jlmat2.setHorizontalAlignment(JLabel.CENTER);
		
		contsm.add(jlmat2);
		contsm.add(jlmat1);
		
		jfmat.setVisible(true);
	}
	public int getTime() {
		notouch = 1;
		updateDisp("800");
...
public void setLimitTime(Player player) {
		...
		label1.setFont(new Font("Century",Font.BOLD,24));
	...
	//後手用）承認画面の表示メソッド
	 void accept(Player player) {
			...
			jfa.setTitle("承認しますか？");
			jfa.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//ウィンドウを閉じる場合の処理
			jfa.setSize(500, 400);//ウィンドウのサイズを設定
			conts = jfa.getContentPane();
			conts.setLayout(null);
			...
			conts.add(access_restTimetext);
			conts.add(access_restTimenum);
