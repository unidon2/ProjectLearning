...
public class Client2 extends JFrame implements MouseListener,ActionListener {
	...
	private Othell game; //Othelloオブジェクト
	...
	// コンストラクタ
	public Client2(Othell game, PlayerSample1 player) { //OthelloオブジェクトとPlayerオブジェクトを引数とする
		this.game = game; //引数のOthelloオブジェクトを渡す
		this.player = player; //引数のPlayerオブジェクトを渡す
		String [] grids = ItoS(game.get_board()); //getGridメソッドにより局面情報を取得
		...
		for(int i = 0 ; i < row * row ; i++){
			...
			turn = new JLabel(game.get_turn()+"のターン");//ターンラベルを作成
			c.add(turn); //貼り付け
	...
	public void updateDisp(String place){	// 画面を更新する
		//c.setVisible(false);
		if(game.put(game.get_turn(),Integer.valueOf(place)/8,Integer.valueOf(place)%8)==1) {
			return;
		}
		game.reload();
		game.print();
		String str[] = ItoS(game.get_board());
			for(int i=0;i<64;i++) {
	...
	public void acceptOperation(String command,Container c){	// プレイヤの操作を受付
		/*if(game.get_turn()!=player.getColor()) {
			c.setVisible(false);
		}
		else
			c.setVisible(true);*/
	...
	public static void main(String args[]){
		...
		Othell game = new Othell(); //オセロオブジェクトを用意
		Client2 oclient = new Client2(game, player); //引数としてオセロオブジェクトを渡す
